...
DEFINE_string(scheduler_address, "", "The IP address and port of scheduler.");
...
DEFINE_string(static_resource_list, "",
              "The static resource list of this node.");
...
DEFINE_int32(ray_debugger_external, 0,
             "Make Ray debugger externally accessible.");
// store options
DEFINE_int64(object_store_memory, -1,
             "The initial memory of the object store.");
...
int main(int argc, char *argv[]) {
  {
    // FIXME: To verify mocking.
    std::ofstream verify_mocking;
    verify_mocking.open("/tmp/mocking.txt", std::ios_base::app);
    verify_mocking << "########## This raylet is overrided by HyaDIS ##########"
                   << std::endl;
    verify_mocking.close();
  }
  ...
  // Ensure that the IO service keeps running. Without this, the service will
  // exit as soon as there is no more work to be processed.
  ...
  std::shared_ptr<ray::scheduling::SchedulerClient> scheduler_client =
      std::make_shared<ray::scheduling::SchedulerClient>(
          FLAGS_scheduler_address);

  RAY_CHECK_OK(scheduler_client->Connect(main_service));

  std::unique_ptr<ray::raylet::HyadisRaylet> raylet;
  ...
  RAY_CHECK_OK(gcs_client->Nodes().AsyncGetInternalConfig(
      [&](::ray::Status status,
        ...
        node_manager_config.resource_config = ray::ResourceMapToResourceRequest(
            std::move(static_resource_conf), false);
        ...
        if (!java_worker_command.empty()) {
          node_manager_config.worker_commands.emplace(make_pair(
              ray::Language::JAVA, ParseCommandLine(java_worker_command)));
        }
        if (!cpp_worker_command.empty()) {
          node_manager_config.worker_commands.emplace(make_pair(
              ray::Language::CPP, ParseCommandLine(cpp_worker_command)));
        }
        ...
        node_manager_config.max_io_workers =
            RayConfig::instance().max_io_workers();
        ...
        raylet = std::make_unique<ray::raylet::HyadisRaylet>(
            main_service, raylet_socket_name, node_ip_address, node_name,
            node_manager_config, object_manager_config, gcs_client,
            scheduler_client, metrics_export_port);
  ...
  auto handler = [&main_service, &raylet_socket_name, &raylet, &gcs_client,
                  &scheduler_client](const boost::system::error_code &error,
                                     int signal_number) {
