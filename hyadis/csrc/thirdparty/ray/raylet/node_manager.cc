...
std::string WorkerOwnerString(std::shared_ptr<WorkerInterface> &worker) {
  ...
  const auto owner_node_id =
      NodeID::FromBinary(worker->GetOwnerAddress().raylet_id());
...
void HeartbeatSender::Heartbeat() {
  ...
  if (interval >
      RayConfig::instance().num_heartbeats_warning() *
          RayConfig::instance().raylet_heartbeat_period_milliseconds()) {
    ...
    : self_node_id_(self_node_id),
      ...
      object_manager_(
          ...
          [this]() {
            ...
            io_service_.post([this]() { TriggerGlobalGC(); },
      ...
      high_plasma_storage_usage_(
          RayConfig::instance().high_plasma_storage_usage()),
      local_gc_run_time_ns_(absl::GetCurrentTimeNanos()),
      local_gc_throttler_(RayConfig::instance().local_gc_min_interval_s() *
                          1e9),
      global_gc_throttler_(RayConfig::instance().global_gc_min_interval_s() *
                           1e9),
  ...
  agent_manager_ = std::make_shared<AgentManager>(
      ...
      [this](const std::string &ip_address, int port) {
        ...
        return std::shared_ptr<rpc::RuntimeEnvAgentClientInterface>(
            new rpc::RuntimeEnvAgentClient(ip_address, port,
...
ray::Status NodeManager::RegisterGcs() {
    ...
    const auto &resources_changed =
        [this](const rpc::NodeResourceChange &resource_notification) {
          ...
          if (resource_notification.deleted_resources_size() != 0) {
            ResourceDeleted(id, VectorFromProtobuf(
                                    resource_notification.deleted_resources()));
          }
        };
  ...
  periodical_runner_.RunFnPeriodically(
      [this] { RecordMetrics(); }, record_metrics_period_ms_,
...
// TODO(mehrdadn): implement graceful process termination mechanism
  ...
  retry_timer->async_wait(
      [retry_timer, worker](const boost::system::error_code &error) {
        RAY_LOG(DEBUG) << "Send SIGKILL to worker, pid="
...
void NodeManager::HandleJobStarted(const JobID &job_id,
                                   const JobTableData &job_data) {
...
void NodeManager::HandleJobFinished(const JobID &job_id,
                                    const JobTableData &job_data) {
...
void NodeManager::FillResourceReport(rpc::ResourcesData &resources_data) {
  ...
  if ((should_local_gc_ ||
       (absl::GetCurrentTimeNanos() - local_gc_run_time_ns_ >
  ...
  for (const auto &worker : all_workers) {
    ...
    worker->rpc_client()->LocalGC(
        request, [](const ray::Status &status, const rpc::LocalGCReply &r) {
          if (!status.ok()) {
            RAY_LOG(DEBUG) << "Failed to send local GC request: "
                           << status.ToString();
          }
        });
  }
...
void NodeManager::HandleReleaseUnusedBundles(
  ...
  std::vector<std::shared_ptr<WorkerInterface>>
      workers_associated_with_unused_bundles;
...
void NodeManager::HandleGetTasksInfo(
    const rpc::GetTasksInfoRequest &request, rpc::GetTasksInfoReply *reply,
    rpc::SendReplyCallback send_reply_callback) {
  QueryAllWorkerStates(
      /*on_replied*/
      [reply](const ray::Status &status,
...
void NodeManager::HandleGetObjectsInfo(
    const rpc::GetObjectsInfoRequest &request, rpc::GetObjectsInfoReply *reply,
    rpc::SendReplyCallback send_reply_callback) {
  QueryAllWorkerStates(
      /*on_replied*/
      [reply](const ray::Status &status,
...
void NodeManager::QueryAllWorkerStates(
    const std::function<void(const ray::Status &,
                             const rpc::GetCoreWorkerStatsReply &)> &on_replied,
    rpc::SendReplyCallback &send_reply_callback, bool include_memory_info,
    bool include_task_info) {
  auto all_workers =
      worker_pool_.GetAllRegisteredWorkers(/* filter_dead_worker */ true);
  for (auto driver :
...
void NodeManager::WarnResourceDeadlock() {
  ...
  if (any_pending && resource_deadlock_warned_++ > 0) {
    ...
    error_message
        << "The actor or task with ID "
        << exemplar.GetTaskSpecification().TaskId()
...
void NodeManager::NodeAdded(const GcsNodeInfo &node_info) {
  ...
  remote_node_manager_addresses_[node_id] = std::make_pair(
  ...
  RAY_CHECK_OK(gcs_client_->NodeResources().AsyncGetResources(
      ...
      [this, node_id](
          Status status,
          const boost::optional<gcs::NodeResourceInfoAccessor::ResourceMap>
              &data) {
        if (data) {
          ResourceRequest resources;
          for (auto &resource_entry : *data) {
            resources.Set(
                scheduling::ResourceID(resource_entry.first),
...
void NodeManager::NodeRemoved(const NodeID &node_id) {
  ...
  if (node_id == self_node_id_) {
    if (!is_node_drained_) {
      RAY_LOG(FATAL)
          << "[Timeout] Exiting because this node manager has mistakenly been "
             "marked as "
             "dead by the "
  ...
  if (!cluster_resource_scheduler_->GetClusterResourceManager().RemoveNode(
    ...
    RAY_LOG(DEBUG) << "Received NodeRemoved callback for an unknown node: "
                   << node_id << ".";
    return;
  }
...
void NodeManager::HandleUnexpectedWorkerFailure(
    const rpc::WorkerDeltaData &data) {
  ...
  for (const auto &pair : leased_workers_) {
    ...
    const auto owner_node_id =
        NodeID::FromBinary(worker->GetOwnerAddress().raylet_id());
    RAY_LOG(DEBUG) << "Lease " << worker->WorkerId() << " owned by "
                   << owner_worker_id;
    ...
    if (!worker->IsDetachedActor()) {
      ...
      if (!worker_id.IsNil()) {
        ...
        // leased worker.
        RAY_LOG(INFO) << "Owner node " << owner_node_id
...
void NodeManager::ResourceCreateUpdated(
    const NodeID &node_id, const ResourceRequest &createUpdatedResources) {
  RAY_LOG(DEBUG) << "[ResourceCreateUpdated] received callback from node id "
  ...
  for (const auto &resource_id : createUpdatedResources.ResourceIds()) {
    cluster_resource_scheduler_->GetClusterResourceManager()
        .UpdateResourceCapacity(
            scheduling::NodeID(node_id.Binary()), resource_id,
...
void NodeManager::ResourceDeleted(
  ...
  if (RAY_LOG_ENABLED(DEBUG)) {
    ...
    RAY_LOG(DEBUG) << "[ResourceDeleted] received callback from node id "
                   << node_id << " with deleted resources: " << oss.str()
                   << ". Updating resource map. skip="
...
void NodeManager::ProcessClientMessage(
  ...
  RAY_LOG(DEBUG) << "[Worker] Message "
                 ...
                 << (registered_worker
                         ? std::to_string(
                               registered_worker->GetProcess().GetId())
                         : "nil");
  ...
  switch (message_type_value) {
    ...
    case protocol::MessageType::NotifyUnblocked: {
      ...
      auto message =
          flatbuffers::GetRoot<protocol::NotifyUnblocked>(message_data);
...
void NodeManager::ProcessRegisterClientRequestMessage(
  ...
  rpc::WorkerType worker_type =
      static_cast<rpc::WorkerType>(message->worker_type());
  ...
  auto worker = std::dynamic_pointer_cast<WorkerInterface>(
      std::make_shared<Worker>(job_id, runtime_env_hash, worker_id, language,
                               worker_type, worker_ip_address, client,
                               client_call_manager_, worker_startup_token));

  auto send_reply_callback = [this, client, job_id](Status status,
                                                    int assigned_port) {
    ...
    auto cb = [this, worker_ip_address, pid, job_id, job_config,
      ...
      if (status.ok()) {
        ...
        RAY_CHECK_OK(gcs_client_->Jobs().AsyncAdd(
            job_data_ptr, [send_reply_callback = std::move(send_reply_callback),
                           assigned_port](Status status) {
...
void NodeManager::ProcessAnnounceWorkerPortMessage(
  ...
  RAY_CHECK(worker != nullptr)
      << "No worker exists for CoreWorker with client: "
      << client->DebugString();

  auto message =
      flatbuffers::GetRoot<protocol::AnnounceWorkerPort>(message_data);
...
void NodeManager::DisconnectClient(
  ...
  RAY_LOG(INFO) << "NodeManager::DisconnectClient, disconnect_type="
                << disconnect_type << ", has creation task exception = "
  ...
  if (worker) {
    ...
    if (worker) {
      ...
      RAY_LOG(INFO)
          << "Ignoring client disconnect because the client has already "
          << "been disconnected.";
      return;
    }
  }
  ...
  if (is_worker) {
    ...
    if ((!task_id.IsNil() || !actor_id.IsNil()) && !worker->IsDead()) {
      ...
      if (disconnect_type == rpc::WorkerExitType::SYSTEM_ERROR_EXIT) {
        ...
        std::ostringstream error_message;
        error_message
            << "A worker died or was killed while executing a task by an "
               "unexpected system "
        ...
        RAY_EVENT(ERROR, EL_RAY_WORKER_FAILURE)
            ...
            << error_message_str;
        auto error_data_ptr = gcs::CreateErrorTableData(
            type, error_message_str, current_time_ms(), job_id);
        RAY_CHECK_OK(
            gcs_client_->Errors().AsyncReportJobError(error_data_ptr, nullptr));
      }
    }
...
void NodeManager::ProcessDisconnectClientMessage(
  ...
  auto disconnect_type =
      static_cast<rpc::WorkerExitType>(message->disconnect_type());
  ...
  if (exception_pb != nullptr) {
    ...
    creation_task_exception->ParseFromString(
        std::string(reinterpret_cast<const char *>(exception_pb->data()),
                    exception_pb->size()));
  }
...
void NodeManager::ProcessFetchOrReconstructMessage(
    ...
    const uint8_t *message_data) {
  auto message =
      flatbuffers::GetRoot<protocol::FetchOrReconstruct>(message_data);
  const auto refs = FlatbufferToObjectReference(*message->object_ids(),
...
void NodeManager::ProcessWaitRequestMessage(
  ...
  uint64_t num_required_objects =
      static_cast<uint64_t>(message->num_ready_objects());
  wait_manager_.Wait(
      ...
      [this, resolve_objects, was_blocked, client, current_task_id](
        ...
        flatbuffers::Offset<protocol::WaitReply> wait_reply =
            protocol::CreateWaitReply(fbb, to_flatbuf(fbb, ready),
...
void NodeManager::ProcessWaitForDirectActorCallArgsRequestMessage(
  ...
  // Read the data.
  auto message =
      flatbuffers::GetRoot<protocol::WaitForDirectActorCallArgsRequest>(
          message_data);
  std::vector<ObjectID> object_ids =
      from_flatbuf<ObjectID>(*message->object_ids());
  ...
  wait_manager_.Wait(object_ids, -1, object_ids.size(),
                     [this, client, tag](std::vector<ObjectID> ready,
                       ...
                       if (!worker) {
                         RAY_LOG(ERROR)
                             << "Lost worker for wait request " << client;
                       } else {
...
void NodeManager::HandleUpdateResourceUsage(
  ...
  resource_usage_batch.ParseFromString(
  ...
  if (next_resource_seq_no_ != 0 &&
    ...
    if (resource_usage_batch.seq_no() < next_resource_seq_no_) {
      RAY_LOG(WARNING)
          << "Discard the the resource update since local version is newer";
      return;
    }
  }
...
void NodeManager::HandleRequestResourceReport(
  ...
  resources_data->set_cluster_full_of_actors_detected(
      resource_deadlock_warned_ >= 1);
...
void NodeManager::HandleGetResourceLoad(
  ...
  auto resources_data = reply->mutable_resources();
  resources_data->set_node_id(self_node_id_.Binary());
  resources_data->set_node_manager_address(
...
void NodeManager::HandleRequestWorkerLease(
      ...
      [this, is_actor_creation_task, actor_id, reply, send_reply_callback](
        ...
        if (reply->rejected() && is_actor_creation_task) {
          ...
          if (RayConfig::instance().gcs_actor_scheduling_enabled()) {
            ...
            resources_data->set_resources_normal_task_changed(true);
            auto resource_map = normal_task_resources.ToResourceMap();
            resources_data->mutable_resources_normal_task()->insert(
                resource_map.begin(), resource_map.end());
            resources_data->set_resources_normal_task_timestamp(
                absl::GetCurrentTimeNanos());
          }
        }
  ...
  cluster_task_manager_->QueueAndScheduleTask(
      task, request.grant_or_reject(), request.is_selected_based_on_locality(),
...
void NodeManager::HandlePrepareBundleResources(
  ...
  auto prepared =
      placement_group_resource_manager_->PrepareBundles(bundle_specs);
...
void NodeManager::HandleShutdownRaylet(
    const rpc::ShutdownRayletRequest &request, rpc::ShutdownRayletReply *reply,
    rpc::SendReplyCallback send_reply_callback) {
  ...
  if (is_node_drained_) {
    RAY_LOG(INFO)
        << "Node already has received the shutdown request. The shutdown "
...
void NodeManager::HandleReleaseUnusedWorkers(
  ...
  for (auto &iter : leased_workers_) {
    ...
    if (!iter.second->GetActorId().IsNil() &&
        !in_use_worker_ids.count(iter.first)) {
  ...
  for (const auto &ref : objects_to_fail) {
    ...
    if (!status.ok() && !status.IsObjectExists()) {
      ...
      stream << "A plasma error (" << status.ToString()
             ...
             << " error code to object " << object_id
             << ". Anyone who's getting this"
             << " object may hang forever.";
      std::string error_message = stream.str();
      RAY_LOG(ERROR) << error_message;
      auto error_data_ptr = gcs::CreateErrorTableData(
          "task", error_message, current_time_ms(), job_id);
      RAY_CHECK_OK(
          gcs_client_->Errors().AsyncReportJobError(error_data_ptr, nullptr));
    }
  }
...
bool NodeManager::FinishAssignedTask(
    const std::shared_ptr<WorkerInterface> &worker_ptr) {
...
void NodeManager::HandleObjectLocal(const ObjectInfo &object_info) {
  ...
  auto waiting_workers =
      absl::flat_hash_set<std::shared_ptr<WorkerInterface>>();
  {
...
void NodeManager::HandleObjectMissing(const ObjectID &object_id) {
  ...
  const auto waiting_task_ids =
      dependency_manager_.HandleObjectMissing(object_id);
...
void NodeManager::ProcessSubscribePlasmaReady(
  ...
  auto message =
      flatbuffers::GetRoot<protocol::SubscribePlasmaReady>(message_data);
  ...
  if (dependency_manager_.CheckObjectLocal(id)) {
    ...
    RAY_LOG(DEBUG) << "Object " << id
                   << " is already local, firing callback directly.";
...
std::string NodeManager::DebugString() const {
  ...
  result << "\nInitialConfigResources: "
         << initial_config_.resource_config.DebugString();
...
void NodeManager::HandlePinObjectIDs(
  ...
  if (!GetObjectsFromPlasma(object_ids, &results)) {
    ...
    send_reply_callback(Status::Invalid("Failed to get objects."), nullptr,
                        nullptr);
    return;
  }
...
void NodeManager::HandleGetNodeStats(
  ...
  for (const auto &view : opencensus::stats::StatsExporter::GetViewData()) {
    ...
    if (view.second.type() == opencensus::stats::ViewData::Type::kInt64) {
      ...
      RAY_CHECK(view.second.type() ==
                opencensus::stats::ViewData::Type::kDistribution);
...
rpc::ObjectStoreStats AccumulateStoreStats(
  ...
  for (const auto &reply : node_stats) {
    ...
    store_stats.set_spill_time_total_s(std::max(
        store_stats.spill_time_total_s(), cur_store.spill_time_total_s()));
    store_stats.set_restore_time_total_s(std::max(
        store_stats.restore_time_total_s(), cur_store.restore_time_total_s()));
...
void NodeManager::PublishInfeasibleTaskError(const RayTask &task) const {
  ...
  if (!suppress_warning) {
    ...
    std::ostringstream error_message;
    error_message << "The actor or task with ID "
